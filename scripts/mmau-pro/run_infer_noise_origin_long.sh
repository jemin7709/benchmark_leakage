...
run_cmd "Qwen3-Omni-Thinking with noise" bash -c "CUDA_VISIBLE_DEVICES=$1 uv run src/inference.py --model qwen3-omni-thinking --noise-path ./assets/white-noise-358382-1m.mp3 --batch-size 100"
run_cmd "Qwen3-Omni-Instruct with noise" bash -c "CUDA_VISIBLE_DEVICES=$1 uv run src/inference.py --model qwen3-omni-instruction --noise-path ./assets/white-noise-358382-1m.mp3 --batch-size 100"
